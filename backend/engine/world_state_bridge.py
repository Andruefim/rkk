...
RKK_WORLD_BRIDGE_LABEL_EVERY=1  — собирать метки не каждый тик, а каждый N-й (остальные — кэш)
"""
...
def _graph_state_vec(sim) -> list[float]:
    g = sim.agent.graph
    node_ids = g._node_ids if hasattr(g, "_node_ids") else list(g.nodes.keys())
    return [float(g.nodes.get(n, 0.5)) for n in node_ids]


def _delta_l2(a: list[float], b: list[float]) -> float:
    """Без numpy — меньше аллокаций на горячем пути."""
    s = 0.0
    for x, y in zip(a, b):
        d = float(x) - float(y)
        s += d * d
...
def collect_grounded_semantic_labels(
    sim,
    ...
    # Эвристика по устойчивости — obs из тика симуляции (без второго observe())
    try:
        o = obs if obs is not None else dict(sim.agent.env.observe())
        ps = float(
            o.get("posture_stability", o.get("phys_posture_stability", 0.5))
        )
        cz = float(o.get("com_z", o.get("phys_com_z", 0.8)))
...
class WorldStateBridge:
    ...
    def __init__(self) -> None:
        ...
        self._label_every = max(1, _env_int("RKK_WORLD_BRIDGE_LABEL_EVERY", 1, min_val=1))
        self._last_labels: list[str] = []

    def on_tick(self, sim, tick_obs: dict[str, float] | None = None) -> None:
        ...
        if self._label_every <= 1 or (tick % self._label_every) == 0:
            self._last_labels = collect_grounded_semantic_labels(sim, obs=tick_obs)
        ...
        if self._prev_state is not None and self._prev_tick >= 0 and tick == self._prev_tick + 1:
            d = _delta_l2(self._prev_state, state_vec)
