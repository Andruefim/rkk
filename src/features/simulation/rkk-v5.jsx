...
function normalizeFrame(raw) {
  ...
  return {
    ...
    agents, demon:raw.demon??{energy:1,cooldown:0,last_action_complexity:0},
...
export default function RKKv5() {
  ...
  const injectSeeds = async () => {
    try {
      ...
      setSeedStatus(`✓ Injected ${d.injected} → ${d.agent}`);
  ...
  useEffect(() => {
    ...
    const agentGroups = AGENT_COLORS_HEX.map((col,i) => {
      ...
      const body  = new THREE.Mesh(new THREE.SphereGeometry(0.6,22,22),
        new THREE.MeshPhongMaterial({color:col,emissive:col,emissiveIntensity:0.35,transparent:true,opacity:0.92}));
      const ring  = new THREE.Mesh(new THREE.TorusGeometry(0.95,0.05,8,44),
        new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:0.4}));
      const ringS = new THREE.Mesh(new THREE.TorusGeometry(1.4,0.025,6,40),
        new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:0.15}));
      // Byzantine consensus ring — синий, пульсирует при активном консенсусе
      const byz   = new THREE.Mesh(new THREE.TorusGeometry(1.7,0.02,5,36),
        new THREE.MeshBasicMaterial({color:0x004488,transparent:true,opacity:0}));
      const shield= new THREE.Mesh(new THREE.TorusGeometry(1.1,0.04,6,40),
        new THREE.MeshBasicMaterial({color:0xff4422,transparent:true,opacity:0}));
      shield.rotation.x = Math.PI/3;
      ring.rotation.x   = Math.PI/2;
      g.add(body); g.add(ring); g.add(ringS); g.add(byz); g.add(shield);
      g.add(new THREE.PointLight(col, 0.7, 5));
      g.position.set((i-1)*6+(Math.random()-0.5),1,(Math.random()-0.5)*3);
      g.userData = {body,ring,ringS,byz,shield,vel:new THREE.Vector3((Math.random()-.5)*0.04,0,(Math.random()-.5)*0.04)};
      scene.add(g); return g;
    });

    const demon = new THREE.Mesh(new THREE.OctahedronGeometry(0.8,0),
    ...
    // Byzantine consensus connection lines (между агентами, синие)
    const byzLines = [];
    for(let a=0;a<3;a++) for(let b=a+1;b<3;b++){
      const geom=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(),new THREE.Vector3()]);
      const line=new THREE.Line(geom,new THREE.LineBasicMaterial({color:0x002255,transparent:true,opacity:0}));
      line.userData={a,b}; scene.add(line); byzLines.push(line);
    }
    const tomLines=[];
    for(let a=0;a<3;a++) for(let b=a+1;b<3;b++){
      const geom=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(),new THREE.Vector3()]);
    ...
    function loop(){
      ...
      const byzPulseOp = byzActive ? Math.max(0, Math.sin(byzPulse*0.08)*0.25) : 0;
      ...
      agentGroups.forEach((g,i)=>{
        ...
        // Byzantine ring медленно вращается, пульсирует при активных раундах
        g.userData.byz.rotation.y  +=0.003;
        g.userData.byz.material.opacity = byzPulseOp;
      ...
      // Byzantine lines (синие, только при активных консенсусных раундах)
      byzLines.forEach(line=>{
        ...
        line.material.opacity=byzActive?byzPulseOp*0.6:0;
      });
    ...
    <div style={{position:"relative",width:"100%",height:"100vh",background:"#010810",overflow:"hidden",...mono}}>
      ...
      <div style={{position:"absolute",top:14,left:"50%",transform:"translateX(-50%)",background:"rgba(0,12,28,0.9)",border:"1px solid #0a2a44",padding:"8px 22px",textAlign:"center",borderRadius:2,boxShadow:"0 0 24px #00224455",whiteSpace:"nowrap"}}>
        <div style={{color:"#00ff99",fontSize:13,fontWeight:"bold",letterSpacing:"0.18em"}}>
          RKK v5 — BYZANTINE · VALUE LAYER · NOTEARS
        </div>
        <div style={{color:"#115577",fontSize:10,marginTop:3,letterSpacing:"0.1em"}}>
          ...
          {ui.byzantine&&<span style={{color:"#004466",marginLeft:6}}>🗳R{ui.byzantine.round}</span>}
          {ui.motif?.last_donor!=null&&<span style={{color:"#224466",marginLeft:6}}>🧬→{["N","A","L"][ui.motif.last_donor]}</span>}
        </div>
      </div>

      {/* Controls */}
      <div style={{position:"absolute",top:82,left:"50%",transform:"translateX(-50%)",background:"rgba(0,8,20,0.85)",border:"1px solid #081e30",padding:"6px 14px",borderRadius:2,display:"flex",gap:8,alignItems:"center",fontSize:9}}>
        <span style={{color:"#224455"}}>SPEED</span>
        {[1,2,4,8].map(s=>(
          <button key={s} onClick={()=>setSpeed(s)} style={{padding:"2px 8px",borderRadius:2,fontSize:9,cursor:"pointer",background:speed===s?"#001e38":"transparent",border:`1px solid ${speed===s?"#00aaff":"#081e30"}`,color:speed===s?"#00aaff":"#334455"}}>{s}×</button>
        ))}
        <span style={{color:"#334455"}}>│</span>
        {PHASE_NAMES.slice(1).map((name,i)=>{const ph=i+1;return(
          <div key={ph} style={{padding:"2px 7px",borderRadius:2,fontSize:9,background:ui.phase===ph?"#001e38":"transparent",border:`1px solid ${ui.phase===ph?"#00aaff":ui.phase>ph?"#003322":"#081e30"}`,color:ui.phase===ph?"#00aaff":ui.phase>ph?"#00aa55":"#223344"}}>{ui.phase>ph?"✓":ph} {name}</div>
        );})}
        <span style={{color:"#334455"}}>│</span>
        <button onClick={()=>setShowSeeds(v=>!v)} style={{padding:"2px 8px",borderRadius:2,fontSize:9,cursor:"pointer",background:showSeeds?"#1a1000":"transparent",border:"1px solid #443300",color:"#886600"}}>💉</button>
      </div>

      {/* Seeds panel */}
      {showSeeds&&(
        <div style={{position:"absolute",top:120,left:"50%",transform:"translateX(-50%)",background:"rgba(8,6,0,0.95)",border:"1px solid #443300",padding:"12px 16px",borderRadius:2,width:400,zIndex:10}}>
          <div style={{color:"#886600",fontSize:10,marginBottom:6}}>💉 LLM/RAG SEED INJECTION</div>
          <div style={{display:"flex",gap:6,marginBottom:6}}>
            {AGENT_NAMES.map((name,i)=>(
              <button key={i} onClick={()=>setSeedAgent(i)} style={{padding:"2px 8px",borderRadius:2,fontSize:9,cursor:"pointer",background:seedAgent===i?"#221100":"transparent",border:`1px solid ${seedAgent===i?AGENT_COLORS_CSS[i]:"#332200"}`,color:seedAgent===i?AGENT_COLORS_CSS[i]:"#554400"}}>{name}</button>
            ))}
          </div>
      ...
      <div style={{position:"absolute",top:120,left:14,display:"flex",flexDirection:"column",gap:8}}>
        {ui.agents.map((a,i)=>{
          ...
          return(
            <div key={i} style={{background:"rgba(0,8,20,0.93)",border:`1px solid ${hasBlock?"#882200":col+"33"}`,borderLeft:`3px solid ${hasBlock?"#ff4422":col}`,padding:"9px 13px",minWidth:225,borderRadius:2,boxShadow:`0 0 14px ${hasBlock?"#ff442218":col+"15"}`,transition:"border-color 0.3s"}}>
              ...
              <table style={{borderCollapse:"collapse",width:"100%",fontSize:10}}><tbody>
                {[
                  ...
                  ["do()",<span style={{color:"#334466",fontSize:9}}>{a.lastDo}</span>],
                ].map(([lbl,val],k)=>(
                  <tr key={k}><td style={{color:"#335566",paddingRight:8,paddingBottom:2}}>{lbl}</td><td style={{color:"#aad4ee",textAlign:"right"}}>{val}</td></tr>
                ))}
              </tbody></table>
              ...
              {a.valueLayer&&(
                <div style={{...sep,fontSize:9}}>
                  ...
                  {hasBlock&&<div style={{color:"#ff6644",marginTop:2}}>⚠ {a.lastBlockedReason}</div>}
                  <div style={{color:"#332211",fontSize:8,marginTop:1}}>
                    phase:{a.valueLayer.vl_phase} str:{a.valueLayer.vl_strictness?.toFixed(2)}
                  </div>
                </div>
              )}
              ...
              {/* Temporal + S1 (компактно) */}
              {(a.temporal||a.system1)&&(
                <div style={{...sep,fontSize:9,color:"#224455"}}>
                  {a.temporal&&<div style={{display:"flex",justifyContent:"space-between"}}>
                    <span>SSM f:{a.temporal.fast_steps} s:{a.temporal.slow_steps}</span>
              ...
              {/* Bars */}
              {[
                {w:alpW, from:"#002266", to:col},
                {w:phiW, from:"#221100", to:phiC},
                {w:`${Math.max(0,100-Math.min((a.hW??0)*20,100))}%`, from:"#001800", to:hWc},
                {w:`${Math.min(blkR*100,100)}%`, from:"#110000", to:blkC},
              ].map((bar,k)=>(
                <div key={k} style={{marginTop:k===0?6:2,height:k===0?3:2,background:"#061422",borderRadius:2,overflow:"hidden"}}>
                  <div style={{width:bar.w,height:"100%",background:`linear-gradient(90deg,${bar.from},${bar.to})`,transition:"width 0.8s"}}/>
                </div>
              ))}
              <div style={{color:"#1a3344",fontSize:8,marginTop:2,display:"flex",justifyContent:"space-between"}}>
                <span>α·Φ·h(W)·vl</span>
      ...
      {/* Right panel: Legend + Byzantine + Motif */}
      <div style={{position:"absolute",top:120,right:14,background:"rgba(0,8,20,0.93)",border:"1px solid #081e30",padding:"10px 14px",borderRadius:2,fontSize:10,maxWidth:195}}>
        <div style={{color:"#114466",letterSpacing:"0.1em",marginBottom:8,fontSize:9}}>RKK v5 — BYZANTINE</div>
        {AGENT_NAMES.map((name,i)=>(
          <div key={i} style={{color:"#335566",marginBottom:2}}><span style={{color:AGENT_COLORS_CSS[i]}}>●</span> {name}</div>
        ))}
        <div style={{color:"#335566",marginBottom:2}}><span style={{color:"#ff2244"}}>◆</span> Demon</div>
        <div style={{color:"#004466",marginBottom:2}}>— Byzantine link</div>
        <div style={{color:"#884400",marginBottom:2}}>⟳ VL shield</div>

        {/* Byzantine stats */}
        {ui.byzantine&&(
          <div style={{marginTop:8,borderTop:"1px solid #081e30",paddingTop:8}}>
            <div style={{color:"#003355",fontSize:9,marginBottom:4}}>🗳 BYZANTINE CONSENSUS</div>
            <div style={{fontSize:9,display:"flex",justifyContent:"space-between"}}>
              <span style={{color:"#224455"}}>round</span>
              <span style={{color:"#336677"}}>{ui.byzantine.round}</span>
            </div>
            <div style={{fontSize:9,display:"flex",justifyContent:"space-between"}}>
              <span style={{color:"#224455"}}>mean deviance</span>
              <span style={{color:devColor(ui.byzantine.mean_deviance??0)}}>{(ui.byzantine.mean_deviance??0).toFixed(4)}</span>
            </div>
            <div style={{fontSize:8,color:"#1a3344",marginTop:3}}>
              every {50} ticks · Φ-weighted · α↓ on deviance&gt;0.25
            </div>
          </div>
        )}

        {/* Motif Transfer stats */}
        {ui.motif&&(
          <div style={{marginTop:6,borderTop:"1px solid #081e30",paddingTop:6}}>
            <div style={{color:"#002244",fontSize:9,marginBottom:4}}>🧬 MOTIF TRANSFER</div>
            <div style={{fontSize:9,display:"flex",justifyContent:"space-between"}}>
              <span style={{color:"#224455"}}>rounds</span>
              <span style={{color:"#225566"}}>{ui.motif.round}</span>
            </div>
            {ui.motif.last_donor!=null&&(
              <div style={{fontSize:9,display:"flex",justifyContent:"space-between"}}>
                <span style={{color:"#224455"}}>last donor</span>
                <span style={{color:AGENT_COLORS_CSS[ui.motif.last_donor]}}>{AGENT_NAMES[ui.motif.last_donor]}</span>
              </div>
            )}
            <div style={{fontSize:8,color:"#1a3344",marginTop:3}}>
              S1 EMA=0.15 · every {100} ticks
            </div>
          </div>
        )}

        {/* Architecture notes */}
        <div style={{marginTop:8,borderTop:"1px solid #081e30",paddingTop:8,color:"#113344",fontSize:9,lineHeight:1.9}}>
          <div style={{color:"#003355"}}>CONSENSUS:</div>
          <div>W_cons=Σ(W·Φ)/ΣΦ</div>
          <div>dev&gt;0.25 → α_trust↓</div>
          <div style={{color:"#002244",marginTop:3}}>MOTIF:</div>
          <div>S1_new=(1-ε)·S1+(ε)·S1_donor</div>
          <div>cross-domain (diff env)</div>
          <div style={{color:"#331100",marginTop:3}}>MULTI:</div>
          <div>Simulation(multiprocess=True)</div>
          <div>Windows spawn · Queue</div>
        </div>
      </div>

      {/* Event log */}
      <div style={{position:"absolute",bottom:14,left:14,right:14,background:"rgba(0,8,20,0.93)",border:"1px solid #081e30",padding:"10px 14px",borderRadius:2,maxHeight:130,overflow:"hidden"}}>
        <div style={{color:"#113344",fontSize:9,letterSpacing:"0.15em",marginBottom:6}}>
          CAUSAL EVENT STREAM {connected?"— BYZANTINE ACTIVE":"— OFFLINE"}
        </div>
        {ui.events.length===0&&<div style={{color:"#1a3344",fontSize:10}}>Initialising Byzantine consensus…</div>}
        {ui.events.map((ev,i)=>(
          <div key={i} style={{color:ev.color??"#335566",fontSize:10,marginBottom:3,opacity:Math.max(0.15,1-i*0.09),fontWeight:ev.type==="value"||ev.type==="tom"?"bold":"normal"}}>
